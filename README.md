...
An [R-tree](http://en.wikipedia.org/wiki/R-tree) is a commonly used spatial xrtree.index.
...
bookmark needed to be kept for a position in the tree and returned to later to continue traversal. An immutable stack containing
 the node and child xrtree.index of the path nodes came to the rescue here and recursion was abandoned in favour of looping to prevent stack overflow (unfortunately java doesn't support tail recursion!).
...
* 10x faster xrtree.index creation with STR bulk loading ([paper](https://www.researchgate.net/profile/Scott_Leutenegger/publication/3686660_STR_A_Simple_and_Efficient_Algorithm_for_R-Tree_Packing/links/5563368008ae86c06b676a02.pdf)).
...
* use STR bulk loaded R-tree (quadratic splitter or R*-tree heuristics) for large (where xrtree.index creation time is important) or static (where insertion and deletion are not frequent) data sets
